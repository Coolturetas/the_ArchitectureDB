...
router.get('/show/:id', (req, res, next) => {
  const promisePost = Comment.find({ postedIn: req.params.id }).populate(
    'creatorId'
  )
  const promiseWork = Work.findById(req.params.id)
    .populate('trend')
    .populate('architect')

  Promise.all([promiseWork, promisePost])
    .then((data) =>
      res.render('works/works-dets', { works: data[0], posts: data[1] })
    )
